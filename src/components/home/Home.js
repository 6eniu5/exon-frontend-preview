...
const Home = () => {
  ...
  const getStatusProp = (status) => {
    if (status === 1) return { success: true };
    if (status === 0) return { warning: true };
    if (status === 2) return { fail: true };
    return { unknown: true };
  };
    ...
    <>
      ...
      <Container>
        ...
        {mockBills.map((bill) => (
          <DataRow
            key={bill._id}
            {...getStatusProp(bill.status)}
            onClick={() => switchModal(bill)}
          >
            <CheckBoxColumn></CheckBoxColumn>
            <Column>
              <DataValue>{toPersian(bill.allocationId)}</DataValue>
            </Column>
            <Column>
              <DataValue>{toPersian(bill.merchantWeight)}</DataValue>
            </Column>
            <Column>
              <DataValue>{toPersian(bill.bill.weight)}</DataValue>
            </Column>
            <Column>
              <DataValue>{toPersian(bill.bill.date)}</DataValue>
            </Column>
            <Column>
              <DataValue>{toPersian(bill.bill.number + '-' + bill.bill.serial)}</DataValue>
            </Column>
            <Column>
            ...
            <StatusColumn>{getStatusIcon(bill.status)}</StatusColumn>
          </DataRow>
        ))}
      </Container>
      <Modal displayModal={isShowingModal} closeModal={switchModal} bill={selectedBill} />
    </>
  );
};
